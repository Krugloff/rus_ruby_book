...
Используется для упаковывки массива в двоичный текст, с помощью [apppack](форматной строки).  
...
Используется для замены всех элементо фрагмента массива заданного размера или диапазона (изменяется значение объекта). Новые объекты либо ссылаются на переданный аргумент, либо вычисляются в результате выполнения блока.  
`[1, 2, 3].fill 1 # -> [1, 1, 1]`
...
Используется для получения фрагмента массива, содержащего элементы с положительным резульатом итерации.  
...
Используется для объединения двух массивов. При совпадении ключей предпочтение отдается аргументу или резульату выполнения блока.  
...
Используется для группировки элементов. Новая группа начинается с элемента равного переданному аргументу (сравнение выполняется с помощью оператора `===`), или c положительным резульататом итерации.
