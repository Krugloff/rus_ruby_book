...
`::sysopen( path, mode = nil, *perm = nil ) # -> integer`
...
Поток может быть закрыт автоматичсеки сборщиком мусора.
...
Конвейеры - это абстрактная сущность, объединяющая несколько процессов. Данные, записываемые в стандартный поток одним процессом, перенаправляются в стандартный поток другого процесса. В Linux создание конвейеров является главным способом взаимодействия программ запускаемых из терминала. Любой конвейер начинает работу как только появляются данные для обработки.
...
`::pipe( *encoding = nil, options = nil ) # -> array`

`( *encoding = nil, options = nil ) { |read, write| } # -> object`
