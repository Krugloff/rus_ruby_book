...
Метод `.initialize` должен определяться разработчиком. Определяемый метод автоматически становится частным. Инициализируемые в нем переменные экземпляра доступны в теле любого другого метода этого экземпляра.
