...
Вместо идентификатора класса может использоваться псевдопеременная self. Это повышает переносимость кода, определяя метод не для какого-то конкретного, а для любого текущего класса.
...
**Во второй версии Ruby** добавлена возмжность переопределять методы, определенные в классе.
