...
Используется для изменения рагмента текста, совпадающего с переданной группой. Отсутствие совпадений считается исключением.
...
Байт с переданым индексом. Если индекс байта выходит за пределы текста, то возвращается nil.  
`"abc".getbyte 0 # -> 97`
...
Проверrf в тексте наличия совпадений.  
`"abc".include? "ab" # -> true`
...
Ипсользуется для сохранения информации о кодировке текста.  
